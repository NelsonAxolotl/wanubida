import { useState, useCallback, useEffect } from "react";

import End from "./End";
import "./Event8Show.css"; // Assure-toi que le chemin du fichier CSS est correct
import pic20 from "../Pics/20.webp"; // Assure-toi que le chemin de l'image est correct

const Event8Show = () => {
  const [enlargedImage, setEnlargedImage] = useState(null);

  // Précharger l'image pour une meilleure performance
  useEffect(() => {
    const img = new Image();
    img.src = pic20;
  }, []);

  const handleImageClick = useCallback((image) => {
    setEnlargedImage(image);
  }, []);

  const handleCloseImage = useCallback(() => {
    setEnlargedImage(null);
  }, []);

  // IDs de l'événement pour le 18 octobre 2024
  const eventId = "5afe33ba4eeecdbd75e7776f15987751"; // ID d'événement correct
  const sessionId = "10510635"; // ID de session correct

  // URL de réservation
  const reservationUrl = `https://www.billetweb.fr/cabaret-wanubida?eventId=${eventId}&session=${sessionId}`;

  return (
    <>
      <div className="event12">
        <div className="kebab-case-orange">
          <div className="text-section12">
            <h4>La tournée du facteur</h4>
            <p>Vendredi 18 Octobre 2024</p>
            <p>19h</p>
            <span>Château de Machy</span>
            <span>1044 Chemin de Machy</span>
            <span>69380 Chasselay</span>
            <a
              href={reservationUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="cta-button"
            >
              Réserver
            </a>
          </div>
          <div className="frame1">
            <img
              src={pic20}
              alt="photo du spectacle la tournée du facteur"
              onClick={() => handleImageClick(pic20)}
              loading="eager" // Charge l'image immédiatement pour éviter les délais
            />
          </div>
        </div>
      </div>
      {enlargedImage && (
        <div className="overlay" onClick={handleCloseImage}>
          <div className="enlarged-image-container">
            <img src={enlargedImage} alt="Enlarged" />
            <button className="close-button" onClick={handleCloseImage}>
              ×
            </button>
          </div>
        </div>
      )}
      <End />
    </>
  );
};

export default Event8Show;
